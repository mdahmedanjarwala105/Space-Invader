...
def enemy_overlap(new_x_axis: int, new_y_axis: int, existing_axis: list):
    for old_x_axis, old_y_axis in existing_axis:
        distance = math.sqrt(
            ...
            + (math.pow(old_y_axis - new_y_axis, 2))
        )
        if distance < 50:
            return False
    return True
...
def score_Val(x_axis: int, y_axis: int):
...
def collisionChecker(
    enemy_x_axis: int, enemy_y_axis: int, bullet_x_axis: int, bullet_y_axis: int
):
    distance = math.sqrt(
        (math.pow(enemy_x_axis - bullet_x_axis, 2))
        + (math.pow(enemy_y_axis - bullet_y_axis, 2))
    )
    if distance < 27:
        return True
...
def drawBig():
    ...
    screen.blit(
        text,
        (BUTTON_X_Axis - text.get_width() / 2, BUTTON_Y_Axis - text.get_height() / 2),
    )
...
def set_game_value():
    ...
    bullet_state = (
        "ready"  # If the bullet is in ready mode it means it isn't moving yet
    )
    ...
    for i in range(num_of_enemies):
        ...
        while True:
            ...
            if enemy_overlap(
                new_x_axis, new_y_axis, existing_axis_enemies
            ):  # if not overlapping then give the x-axis and y-axis to enemy or else since it's while true it will again give new value and check the overlapping
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            ):  # Keydown means key pressed if we use .KEYUP that means key released
        ...
        if player_x_axis <= 0:
            ...
            player_x_axis >= 736
        ):  # using number 736 because of the player image pixel which is 64 (800 - 64)
            player_x_axis = 736
        ...
        for i in range(num_of_enemies):
            ...
            collision = collisionChecker(
                enemy_x_axis[i], enemy_y_axis[i], bullet_x_axis, bullet_y_axis
            )
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                distance = math.sqrt(
                    (math.pow(BUTTON_X_Axis - mouse_x_axis, 2))
                    + (math.pow(BUTTON_Y_Axis - mouse_y_axis, 2))
                )
